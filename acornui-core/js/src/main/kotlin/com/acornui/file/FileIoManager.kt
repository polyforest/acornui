...
actual class FileIoManager : Disposable {

	actual val saveSupported: Boolean = true
	...
	private fun changeHandler(onSuccess: (Any) -> Unit): (Event) -> Unit = {
		...
		if (filePicker != null) {
			...
			val fileReaders = if (fileList == null || fileList.length == 0) {
				...
				val tempList = mutableListOf<FileReader>()
				for (i in 0..fileList.length - 1) {
					tempList.add(FileReader(fileList[i] ?: continue))
				}
				tempList
			}
			if (fileReaders != null) {
	...
	actual fun pickFileForOpen(fileFilterGroups: List<FileFilterGroup>?, onSuccess: (FileReader) -> Unit) {
		destroyFilePicker()
	...
	actual fun pickFilesForOpen(fileFilterGroups: List<FileFilterGroup>?, onSuccess: (List<FileReader>) -> Unit) {
		destroyFilePicker()
		filePicker = createFilePicker()
	...
	actual fun saveText(text: String, fileFilterGroups: List<FileFilterGroup>?, defaultFilename: String, defaultExtension: String?) {
		saveData(text, defaultFilename)
	}

	actual fun saveBinary(data: NativeReadBuffer<Byte>, fileFilterGroups: List<FileFilterGroup>?, defaultFilename: String, defaultExtension: String?) {
	...
	actual companion object : DKey<FileIoManager>
}

actual class FileReader(val file: DomFile) {

	actual val name = file.name
	actual val size = file.size.toLong()
	actual val lastModified = file.lastModified.toLong()
	private val reader = JsFileApiReader()

	actual suspend fun readAsString(): String {
	...
	actual suspend fun readAsBinary(): NativeReadByteBuffer {
