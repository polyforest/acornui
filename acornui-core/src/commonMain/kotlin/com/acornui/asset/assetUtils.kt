...
fun <T> Scoped.loadJson(deserializer: DeserializationStrategy<T>, path:String): Deferred<T> = async {
	val json = inject(AssetManager).load(path, AssetType.TEXT)
	jsonParse(deserializer, json.await())
}
...
fun <T> Scoped.loadBinary(path: String, factory: From<T>): Deferred<T> = async {
	...
	binaryParse(binary.await(), factory)
}

fun <T> Scoped.loadBinary(deserializer: DeserializationStrategy<T>, path: String): Deferred<T> = async {
	val binary = inject(AssetManager).load(path, AssetType.BINARY)
	binaryParse(deserializer, binary.await().toByteArray())
}