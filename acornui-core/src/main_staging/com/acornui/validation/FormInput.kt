...
open class FormInput<T>(owner: Context) : SingleElementContainerImpl<InputComponent<T>>(owner), InputComponent<T>, Labelable {
	...
	override val changed = signal<InputComponent<T>>()
	...
	override fun updateLayout(explicitBounds: ExplicitBounds): Bounds {
		...
		requiredElement.position(0.0, labelTextField.bottom + style.verticalGap)
	...
	private fun iconY(): Double {
		return requiredElement.y + when (style.verticalAlign) {
			VAlign.TOP -> 0.0
			VAlign.MIDDLE -> (requiredElement.height - icon.height) * 0.5
...
class FormInputStyle : ObservableBase() {
