...
open class Calendar(
	...
	private val panel = addChild(panel {
		+vGroup {
			+hGroup {
				...
				+spacer() layout { widthPercent = 1.0 }
	...
	val style by layoutProp(CalendarStyle())
	...
	override fun updateLayout(explicitBounds: ExplicitBounds): Bounds {
		...
		for (i in 0..6) {
			columns.add(GridColumn(hAlign = style.columnHAlign, widthPercent = 1.0, minWidth = maxHeaderW))
		}
		grid.apply {
			style.columns = columns
			style.rowHeight = if (explicitHeight == null) null else (explicitHeight - 6.0 * style.verticalGap) / 7.0
			cells.forEach {
				if (it.layoutData == null) {
					it.layoutData = grid.createLayoutData()
				}
				(it.layoutData as GridLayoutData).let { layoutData ->
					layoutData.widthPercent = 1.0
					layoutData.heightPercent = if (explicitHeight == null) null else 1.0
				}
			}
		}
...
class CalendarStyle : ObservableBase() {
...
open class CalendarItemRendererImpl(owner: Context) : ContainerImpl(owner), CalendarItemRenderer {
	...
	private val background = addChild(rect {
		...
		style.borderColors = BorderColors(Color(0.0, 0.0, 0.0, 0.3))
	})
...
open class CalendarItemRendererStyle : ObservableBase() {
	...
	var padding by prop(Pad(2.0))

	var disabledColor by prop(Color(0.5, 0.5, 0.5, 0.6))
	var upColor by prop(Color(1.0, 1.0, 1.0, 0.6))
	var overColor by prop(Color(1.0, 1.0, 0.5, 0.6))
	var downColor by prop(Color(0.6, 0.6, 0.5, 0.6))
	var toggledUpColor by prop(Color(1.0, 1.0, 0.0, 0.4))
	var toggledOverColor by prop(Color(1.0, 1.0, 0.0, 0.6))
	var toggledDownColor by prop(Color(1.0, 1.0, 0.0, 0.3))
