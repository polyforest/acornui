...
inline fun Context.strongEmphasizedText(text: String = "", init: ComponentInit<TextField> = {}): TextField {
	contract { callsInPlace(init, InvocationKind.EXACTLY_ONCE) }
	val t = TextFieldImpl(this)
	t.addClassAll(TextStyleTags.emphasis, TextStyleTags.strong)
	t.text = text
	t.init()
	return t
}
...
class CharStyle : DpiStyle() {
	 ...
	 * @see strikeThrough
	 */
	var lineThickness by prop(1.0)

	var colorTint by prop<ColorRo>(Color(1.0, 1.0, 1.0, 1.0))
	var backgroundColor by prop<ColorRo>(Color())

	var selectedColorTint by prop<ColorRo>(Color(1.0, 1.0, 1.0, 1.0))
	var selectedBackgroundColor by prop<ColorRo>(Color(0.12, 0.25, 0.5, 1.0))
...
class TextFlowStyle : ObservableBase() {
...
class TextInputStyle : ObservableBase() {
	override val type = Companion

	var defaultWidth by prop(180.0)

	/**
	 * The cursor blinks, this is color one.
	 */
	var cursorColorOne by prop<ColorRo>(Color(0.1, 0.1, 0.1, 1.0))

	/**
	 * The cursor blinks, this is color two.
	 */
	var cursorColorTwo by prop<ColorRo>(Color(0.9, 0.9, 0.9, 1.0))

	/**
	 * The number of seconds between cursor blinks.
	 */
	var cursorBlinkSpeed by prop(0.5)
