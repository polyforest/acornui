...
interface TextField : SingleElementContainer<TextNode>, Labelable, SelectableComponent, Stylable {
	 ...
	 */
	val fontScaleX: Double

	/**
	 * The y dpi scaling from dp to pixels.
	 */
	val fontScaleY: Double
...
open class TextFieldImpl(owner: Context) : SingleElementContainerImpl<TextNode>(owner), TextField {
	...
	final override var fontScaleX: Double by validationProp(1.0, ValidationFlags.LAYOUT)
		private set

	final override var fontScaleY: Double by validationProp(1.0, ValidationFlags.LAYOUT)
		private set
	...
	init {
		...
		watch(charStyle) { cS ->
			...
			if (cS.selectable) {
				cursor(StandardCursor.TEXT)
				dragAttachment(0.0, DRAG_ATTACHMENT_KEY).drag.add(::dragHandler)
			} else {
				clearCursor()
	...
	override fun updateLayout(explicitBounds: ExplicitBounds): Bounds {
		...
		if (contents.textElements.isEmpty()) {
			...
			val lineHeight: Double = (fontData?.lineHeight?.toDouble() ?: 0.0) / fontScaleY
			out.height = padding.expandHeight(lineHeight)
			if (flowStyle.sizeToContents)
				out.width = padding.left + padding.right
			out.baseline = padding.top + (fontData?.baseline?.toDouble() ?: 0.0) / fontScaleY
