...
class DataGrid<RowData>(
	 ...
	 * Note that if [maxRows] is too large and [minRowHeight] is too small, a very large number of cells may be created.
	 */
	var minRowHeight by validationProp(15.0, ValidationFlags.LAYOUT)
	 ...
	 */
	var rowHeight: Double? by validationProp(null, ValidationFlags.LAYOUT)
	...
	private var columnMoveIndicator = feedback.addElement(rect { addClass(COLUMN_MOVE_INDICATOR); visible = false })
	private var columnInsertionIndicator = feedback.addElement(vr { addClass(COLUMN_INSERTION_INDICATOR); visible = false })

	private val hScrollbar = clipper.addElement(hScrollbar { visible = false; addClass(SCROLL_BAR) })
	private val vScrollbar = clipper.addElement(vScrollbar { visible = false; addClass(SCROLL_BAR) })
	...
	private inner class DataGridTossScrollBinding : TossScrollModelBinding(tossScroller, hScrollModel, vScrollModel) {
		...
		override fun localToModel(diffPoints: Vector2) {
			...
			diffPoints.scl(1.0, 1.0 / firstRowHeight)
		}
	}
	...
	init {
		...
		hScrollbar.scrollModel.snap = 1.0
...
//		event.handled
		...
		if (loc != null) {
			when (event.keyCode) {
				...
				Ascii.PAGE_DOWN -> {
					...
					vScrollModel.value = loc.position.toDouble()
					validate(ValidationFlags.LAYOUT)
			...
			when (event.keyCode) {
				Ascii.HOME -> vScrollModel.value = 0.0
				Ascii.END -> vScrollModel.value = totalRows - 1.0
				...
				Ascii.RIGHT -> hScrollModel.value += 20.0
				Ascii.LEFT -> hScrollModel.value -= 20.0
				else -> return
			}
			event.handled = true
			return
	 ...
	 */
	fun getCellFromPosition(canvasX: Double, canvasY: Double): CellLocationRo<RowData> {
		validate(ValidationFlags.LAYOUT)
		val p = tmp
		...
		if (rowPosition < 0.0) return CellLocation(this, -1, columnIndex)
	...
	fun bringIntoView(rowLocation: RowLocationRo<RowData>) {
		if (rowLocation.position < _cellMetrics.startPosition) {
			vScrollModel.value = rowLocation.position.toDouble()
		} else if (rowLocation.position > _cellMetrics.endPosition - 1.0) {
			sizeCellsReversed(
					width = _bottomCellMetrics.bounds.width,
					height = _bottomCellMetrics.bounds.height,
					endPosition = rowLocation.position.toDouble() + 1.0,
	...
	override fun onSizeSet(oldWidth: Double?, oldHeight: Double?, newWidth: Double?, newHeight: Double?) {
		invalidate(COLUMNS_WIDTHS_VALIDATION)
	}

	private val _columnWidths = ArrayList<Double>()
	...
	private val _columnPositions = ArrayList<Double>()
	...
	private fun updateColumnWidths() {
		...
		val vScrollbarW = if (vScrollPolicy == ScrollPolicy.OFF) 0.0 else vScrollbar.minWidth
		...
		hScrollbar.visible = hScrollPolicy == ScrollPolicy.ON || (hScrollPolicy == ScrollPolicy.AUTO && hScrollbar.scrollModel.max > 0.0)
	}
	 ...
	 */
	private fun adjustColumnWidths(delta: Double, update: Boolean): Double {
		var d = delta
		while (abs(d) > 0.01 && columnsWithSpace.isNotEmpty()) {
	 ...
	 */
	private fun setColumnWidth(columnIndex: Int, newWidth: Double) {
	 ...
	 */
	private fun getPreferredColumnWidth(column: DataGridColumn<RowData, *>): Double {
		if (!column.visible) return 0.0
	...
	override fun updateLayout(explicitBounds: ExplicitBounds): Bounds {
		...
		val bodyW = contentsW + if (vScrollPolicy != ScrollPolicy.OFF) vScrollbar.naturalWidth else 0.0

		out.width = border.expandWidth(bodyW)

		updateHeader(contentsW)

		val hScrollbarH = if (hScrollbar.visible) hScrollbar.naturalHeight else 0.0
		...
		sizeCellsReversed(
				width = contentsW,
				height = contentsH,
				endPosition = totalRows.toDouble(),
		...
		vScrollbar.modelToPoints = bottomBounds.height / maxOf(0.0001, bottomRowCount)
		vScrollbar.scrollModel.max = maxOf(0.0, totalRows - bottomRowCount)
		vScrollbar.visible = vScrollPolicy != ScrollPolicy.OFF && vScrollbar.scrollModel.max > 0.0001
		val vScrollbarW = if (vScrollbar.visible) vScrollbar.naturalWidth else 0.0
		...
		rowBackgrounds.position(0.0, headerCells.height)
		contents.size(contentsW, contentsH)
		contents.position(0.0, headerCells.height)
		editorCellContainer.size(contentsW, contentsH)
		editorCellContainer.position(0.0, headerCells.height)

		groupHeadersAndFooters.size(contentsW, contentsH)
		groupHeadersAndFooters.position(0.0, headerCells.height)
		...
		if (topRight != null) {
			...
			topRight.position(bodyW - topRight.width, 0.0)
		}
		val headerDivider = this.headerDivider
		if (headerDivider != null) {
			headerDivider.size(bodyW, null)
			headerDivider.position(0.0, headerCells.height - headerDivider.height)
		}
	...
	private fun updateHeader(width: Double) {
		...
		iterateVisibleColumnsInternal { columnIndex, column, x, columnWidth ->
			...
			if (columnCache.headerCell == null) {
				...
				newHeaderCell.addClassAll(HEADER_CELL, TextStyleTags.large)
		...
		if (sortArrow != null) {
			sortArrow.y = when (style.headerSortArrowVAlign) {
				...
				VAlign.MIDDLE -> cellPad.top + (headerCellHeight - sortArrow.height) * 0.5
		...
		iterateVisibleColumnsInternal { i, col, colX, colWidth ->
			...
			val headerCell = columnCache.headerCell!!
			headerCell.visible = true
			val y = cellPad.top + maxOf(0.0, when (col.headerCellVAlign ?: style.headerCellVAlign) {
				VAlign.TOP -> 0.0
				VAlign.MIDDLE -> (headerCellHeight - headerCell.height) * 0.5
			...
			val x = cellPad.left + maxOf(0.0, when (col.headerCellHAlign ?: style.headerCellHAlign) {
				HAlign.LEFT -> 0.0
				HAlign.CENTER -> (headerCellWidth - headerCell.width) * 0.5
			...
			headerCellBackground.position(colX, 0.0)
			...
			resizeHandle.position(colWidth + colX - resizeHandle.width * 0.5, 0.0)
		...
		columnResizeHandles.size(width + 5.0, headerHeight)
	...
	private fun sizeCellsReversed(
		...
		var heightSum: Double
		val maxRows = minOf(totalRows, maxRows)
		val maxHeight = height ?: Double.MAX_VALUE

		if (allowVirtual && rowHeight != null) {
			heightSum = minOf(maxHeight, rowHeight * maxRows)
			metrics.startPosition = maxOf(0.0, endPosition - heightSum / rowHeight)
			...
			val overhang = if (heightSum <= maxHeight) 0.0 else {
				(heightSum - maxHeight) / iRowHeight
			}
			metrics.startPosition = rowIterator.position.toDouble() + overhang
	...
	private fun sizeCells(
		...
		var rowsShown = 0
		var heightSum = 0.0
		var iRowHeight: Double = minRowHeight
		...
		if (allowVirtual && rowHeight != null) {
			...
			metrics.endPosition = maxOf(0.0, heightSum / rowHeight - startPosition)
			...
			val overhang = if (heightSum <= height) 0.0 else {
				(heightSum - height) / iRowHeight
			}
			metrics.endPosition = rowIterator.position.toDouble() - overhang
		}
	...
	private fun positionCells() {
		...
		for (i in 0..metrics.rowHeights.lastIndex) {
			...
			if (rowIterator.isHeader) {
				rowIterator.groupCache.header!!.position(0.0, rowPosition)
			} else if (rowIterator.isFooter) {
//				rowIterator.groupCache.footer!!.setPosition(0.0, rowPosition)
			} else {
				iterateVisibleColumnsInternal { columnIndex, column, columnX, columnWidth ->
					@Suppress("unchecked_cast")
					val cell = cellCache.columnCellCaches[columnIndex][cellRow] as DataGridCell<Any?>

					val y = pad.top + maxOf(0.0, when (column.cellVAlign ?: style.cellVAlign) {
						VAlign.TOP -> 0.0
						VAlign.MIDDLE -> (rowHeight - cell.height) * 0.5
						VAlign.BASELINE, VAlign.BOTTOM -> (rowHeight - cell.height)
					})
					val cellWidth = cell.explicitWidth ?: cell.width

					val x = pad.left + maxOf(0.0, when (column.cellHAlign ?: style.headerCellHAlign) {
						HAlign.LEFT -> 0.0
						HAlign.CENTER -> (cellWidth - cell.width) * 0.5
	...
	private fun updateRowBackgrounds() {
		...
		for (i in 0.._cellMetrics.rowHeights.lastIndex) {
			...
			rowBackground.position(0.0, _cellMetrics.rowPositions[i])
		}
	...
	fun iterateVisibleRows(callback: (row: RowLocation<RowData>, rowY: Double, rowHeight: Double) -> Boolean) {
		validate(ValidationFlags.LAYOUT)
	 ...
	 */
	fun iterateVisibleColumns(callback: (columnIndex: Int, column: DataGridColumn<RowData, *>, columnX: Double, columnWidth: Double) -> Boolean) {
	 ...
	 */
	private fun iterateVisibleColumnsInternal(callback: (columnIndex: Int, column: DataGridColumn<RowData, *>, columnX: Double, columnWidth: Double) -> Boolean) {
	...
	private fun createHeaderCellBackground(): UiComponent {
		...
		drag.drag.add { e ->
			columnMoveIndicator.position(e.currentTarget.x + (e.position.x - e.startPosition.x), 0.0)
			...
			if (index > 0 && currX < _columnPositions[index - 1] + _columnWidths[index - 1] * 0.5) index--
			...
			val insertX = (if (index <= 0) 0.0 else if (index >= _columnPositions.size) _columnPositions.last() + _columnWidths.last() else _columnPositions[index]) - hScrollModel.value
			columnInsertionIndicator.position(insertX - columnInsertionIndicator.width * 0.5, 0.0)
			Vector2.free(localP)
		}
		...
		drag.dragEnd.add { e ->
			...
			if (toIndex > 0 && currX < _columnPositions[toIndex - 1] + _columnWidths[toIndex - 1] * 0.5) toIndex--
	...
	private fun createColumnResizeHandle(): UiComponent {
		val resizeHandle = spacer(style.resizeHandleWidth, 0.0)
		resizeHandle.interactivityMode = InteractivityMode.ALL
		resizeHandle.cursor(StandardCursor.EW_RESIZE)

		val drag = resizeHandle.dragAttachment(0.0)
		var colResizeStartX = 0.0
		var columnIndex = 0
		drag.dragStart.add {
		...
		drag.drag.add {
			...
			var newWidth: Double = maxOf(column.minWidth, localP.x - colResizeStartX)
			Vector2.free(localP)
	...
	private fun updateVerticalDividers(width: Double, height: Double) {
		var shownColumns = 0
		iterateVisibleColumnsInternal { columnIndex, column, columnX, columnWidth ->
			...
			headerDivider.position(columnX + columnWidth, 0.0)
...
class DataGridStyle : ObservableBase() {
	 ...
	 */
	var defaultColumnWidth by prop(100.0)

	/**
	 * The width of the column resize handles.
	 */
	var resizeHandleWidth by prop(16.0)
	 ...
	 */
	var headerCellPadding by prop(Pad(4.0))

	/**
	 * The gap between the header cell and the sort arrow (if there is one).
	 */
	var headerCellGap by prop(2.0)
	 ...
	 */
	var cellPadding by prop(Pad(4.0))
