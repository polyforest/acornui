...
class OptionList<E : Any>(
	...
	private val textInput: TextInput = textInput {
		input.add {
			if (openOnInput) open()
			scrollModel.value = 0.0 // Scroll to the top.
	...
	private fun highlightNext(delta: Int) {
		...
		if (nextIndexNotNull != -1) {
			...
			scrollTo(nextIndexNotNull.toDouble())
	...
	private fun highlightPrevious(delta: Int) {
		...
		if (previousIndexNotNull != -1) {
			...
			scrollTo(previousIndexNotNull.toDouble())
			dataScroller.focusSelf()
		}
	}
	...
	private fun highlightLast() {
		...
		if (lastIndexNotNull != -1) {
			...
			scrollTo(lastIndexNotNull.toDouble())
			dataScroller.focusSelf()
		}
	}
	...
	var listWidth: Double? by validationProp(null, ValidationFlags.LAYOUT)
	var listHeight: Double? by validationProp(null, ValidationFlags.LAYOUT)
	...
	override fun updateLayout(explicitBounds: ExplicitBounds): Bounds {
		...
		downArrow.position(pad.left + textInput.width + style.hGap, pad.top + (textInput.height - downArrow.height) * 0.5)
		...
		listLift.position(0.0, out.height + style.vGap)
	}
...
class OptionListStyle : ObservableBase() {
