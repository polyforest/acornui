...
class VirtualHorizontalLayout : VirtualLayoutAlgorithm<VirtualHorizontalLayoutStyle, HorizontalLayoutData> {
	...
	override fun getOffset(width: Double, height: Double, element: LayoutElementRo, index: Int, lastIndex: Int, isReversed: Boolean, props: VirtualHorizontalLayoutStyle): Double {
		val padding = props.padding
		val gap = props.gap
		val elementW = round(element.width)
		return if (isReversed) {
			(width - padding.right - (element.x + elementW)) / maxOf(0.0001, elementW + gap)
		} else {
			(element.x - padding.bottom) / maxOf(0.0001, elementW + gap)
		}
	}

	override fun updateLayoutEntry(explicitWidth: Double?, explicitHeight: Double?, element: LayoutElement, currentIndex: Int, startIndex: Double, lastIndex: Int, previousElement: LayoutElementRo?, isReversed: Boolean, props: VirtualHorizontalLayoutStyle) {
		val padding = props.padding
		val gap = props.gap
		...
		val x = if (previousElement == null) {
			...
			if (isReversed) {
				(childAvailableWidth ?: 0.0) - padding.right + startX - elementW
			} else {
				padding.left + startX
			}
		} else {
		...
		if (childAvailableHeight == null) {
			...
			when (layoutData?.verticalAlign ?: verticalAlign) {
				VAlign.TOP ->
					element.position(x, padding.top)
				VAlign.MIDDLE ->
					element.position(x, padding.top + (childAvailableHeight - element.height) * 0.5)
	...
	override fun measure(explicitWidth: Double?, explicitHeight: Double?, elements: List<LayoutElementRo>, props: VirtualHorizontalLayoutStyle, out: Bounds) {
		val padding = props.padding
	...
	override fun shouldShowRenderer(explicitWidth: Double?, explicitHeight: Double?, element: LayoutElementRo, props: VirtualHorizontalLayoutStyle): Boolean {
...
open class VirtualHorizontalLayoutStyle : ObservableBase() {
