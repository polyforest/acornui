...
open class TabNavigator(owner: Context) : ContainerImpl(owner), LayoutDataProvider<StackLayoutData> {
	 ...
	 */
	val tabBarContainer: UiComponent
	...
	fun <T : TabNavigatorTab> addTab(index: Int, tab: T): T {
		...
		if (newIndex == 0) {
			...
			tab.button.addClass(DEFAULT_TAB_STYLE_FIRST)
		}
		if (newIndex == tabs.size) {
			if (_tabs.size > 0) {
				_tabs.last().button.styleTags.remove(DEFAULT_TAB_STYLE_LAST)
			}
			tab.button.addClass(DEFAULT_TAB_STYLE_LAST)
		}
		tab.button.addClass(DEFAULT_TAB_STYLE)
	...
	fun removeTab(index: Int): TabNavigatorTab {
		...
		if (index == 0) {
			if (_tabs.size > 0) {
				addClass(DEFAULT_TAB_STYLE_FIRST)
			}
	...
	override fun updateLayout(explicitBounds: ExplicitBounds): Bounds {
		...
		background.position(0.0, tabBarHeight)
		out.width = maxOf(background.width, tabBarWidth)
		out.height = background.height + tabBarHeight
	}

	protected open fun updateTabBarLayout(explicitWidth: Double?, explicitHeight: Double?) {
...
class TabNavigatorStyle : ObservableBase() {
	 ...
	 */
	var tabBarPadding by prop(Pad(0.0, 0.0, 0.0, -1.0))
	 ...
	 */
	var contentsPadding by prop(Pad(0.0))
